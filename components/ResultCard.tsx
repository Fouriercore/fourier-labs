...
export const ResultCard: React.FC<ResultCardProps> = ({ result }) => {
  ...
  const handleCopyReport = async () => {
    const reportText = `### Fourier Security Telemetry Report
**Contract Address**: \`${address}\`
**Security Status**: ${status.toUpperCase()}
**Risk Level**: ${riskLevel.toUpperCase()}
**Analysis Confidence**: ${confidence}%
**Threat Category**: ${categoryNames[category]}
**Last Verification**: ${formattedDate}

#### Analysis Reason:
> ${reason}

*Report generated by Fourier — Soroban Smart Contract Reputation Platform.*`;
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-white/5 pb-5">
          ...
          <div className="flex items-center gap-3">
            <button
              ...
              className="inline-flex items-center gap-1.5 rounded bg-white/5 border border-white/10 hover:bg-white/10 hover:border-white/20 px-3 py-1.5 font-mono text-xs font-bold text-gray-300 transition-all duration-200"
              title="Copy markdown report to clipboard"
            >
              {copied ? (
                <>
                  <Check className="h-3.5 w-3.5 text-[#04ed16]" />
                  <span>COPIED</span>
                </>
              ) : (
                <>
